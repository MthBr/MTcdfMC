...
#%% def functions
def pde_moive_function(i, D, mesh, delta_space, dim, flatten_field, timeStepDuration, steps=150, verbose=False):
    #%% 4.  set Param
    ...
    delta= delta_space - delta_space*0.3
    where_delta= (abs(mesh.cellCenters[0]-0.5) < delta)
    if dim < 3 : where_delta = where_delta & (abs(mesh.cellCenters[1]-0.5) < delta)
    phi.setValue(1, where=where_delta )  #dxy*nxy//2
    ...
    for step in range(steps):
        ...
        if not step%20: print(f'i={i}, step = {step} over {steps}')
