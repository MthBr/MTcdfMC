...
from cdf_project.data_etl.etl_utils import array_from_mesh , gen_x_mesh
...
#%%
#test = '1FullTest' #'l-1-1-1d',
#delta_types = ['l-1-0-1d', 'l-1-1-2d', 'l-1-2-2d', 'l-2-2-1d', 'l-2-2-2d', 'l-2-3-1d', 'l-2-3-2d', 'l-2-5-1d', 'l-2-5-2d']


test = '2FullSeq' #'2FullTest' #'l-1-1-1d',
delta_types = ['l-1-1-1d', 'cos-1-1d', 'cos-2-1d', 'cos-2-2d', 'l-1-1-1d-s2', 'cos-1-1d-s2', 'cos-2-1d-s2', 'p-cos-s2', 'p*cos-s25']
...
from cdf_project.tests.delta_related.utils_delta_stats import plot_nsave_solution
...
ksup_arr = [1.0, np.sqrt(2), 1.5, 2.0, 2.5, 3.0]
num_k = len(ksup_arr)
...
for idx, delta_tp  in enumerate(delta_types):
    ...
    for j, ksup in enumerate(ksup_arr):
        ...
        rates[delta_tp+'s'+str(ksup)] = rates_array.tolist()
        ...
        rates_rel[delta_tp+'s'+str(ksup)] = rates_rel_array.tolist()
