...
delta_types = ['p2h-s1', 'p2h-s1', 'p4h-s2', 'p4h-s2']
k = [1,2,0.5,1]
delta_typ_descript = ['p2h-s1', 'p2h-s1', 'p4h-s2', 'p4h-s2']
...
for idx, delta in enumerate(delta_types):
    ...
    delta_value =delta_func(mesh, 0.0, k[idx], dim, delta)
    ax.set_title(f"{delta_typ_descript[idx]}_k{k[idx]} \n min={np.min(delta_value):.1f} \n max={np.max(delta_value):.1f}")
