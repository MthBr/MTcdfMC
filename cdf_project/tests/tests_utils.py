...
from cdf_project.custom_funcs import benchmark
...
def plot_final_funcs_x(num_of_cells, find_anlyt_error_func, timeDuration):
    ...
    errors_nu_chb, analyt_cheb = find_anlyt_error_func(mesh, three_time_phi[1][-1], timeDuration)
    ax=axs[2][2]
    ax.plot(x[0], analyt_cheb, label="analytical")
    ax.plot(x[0], three_time_phi[1][-1], linestyle="--", label="u")
    ax.legend(loc="best")
    ax.set_xlabel("x")
    ax.set_ylabel("u")
    ax.set_title(f'T={timeDuration}, err={errors_nu_chb:.5f}')


    errors_nu_chb, analyt_cheb = find_anlyt_error_func(mesh, three_time_phi[1][-2], timeDuration/2)
    ax=axs[1][2]
    ax.plot(x[0], analyt_cheb, label="analytical")
    ax.plot(x[0], three_time_phi[1][-2], linestyle="--", label="u")
    ax.legend(loc="best")
    ax.set_xlabel("x")
    ax.set_ylabel("u")
    ax.set_title(f'T={timeDuration/2}, err={errors_nu_chb:.5f}')

    errors_nu_chb, analyt_cheb = find_anlyt_error_func(mesh, three_time_phi[1][-3], timeDuration/10)
    ax=axs[0][2]
    ax.plot(x[0], analyt_cheb, label="analytical")
    ax.plot(x[0], three_time_phi[1][-2], linestyle="--", label="u")
    ax.legend(loc="best")
    ax.set_xlabel("x")
    ax.set_ylabel("u")
    ax.set_title(f"Non-unif (Chebyshev) {num_of_cells} mesh\n"+f'T={timeDuration/10}, err={errors_nu_chb:.5f}')

    fig.tight_layout()


    #%% SAVE
    file_name = setup_dict['compactxt'] + f'_{num_of_cells}'
...
def initializer(nxvec):
    ...
    return errors_u, errors_nu_seg, errors_nu_chb

def init_pool(err_u, err_nu_seg, err_nu_chb):
    global errors_u, errors_nu_seg, errors_nu_chb
    errors_u = err_u
    errors_nu_seg = err_nu_seg
    errors_nu_chb = err_nu_chb 
...
def gen_errs(indx, num_of_cells, find_anlyt_error_func, timeDuration):
    ...
    setup_dict['center_spacing']  = max(delta_vect)
    three_time_phi = de_f_time(indx, setup_dict, mesh, None, timeDuration)  #False  True
    errors_nu_chb, _ = find_anlyt_error_func(mesh, three_time_phi[1][-1], timeDuration)
    return indx, errors_u, errors_nu_seg, errors_nu_chb
...
def error_multicore(nxvec, find_anlyt_error_func, timeDuration):
    ...
    global errors_u, errors_nu_seg, errors_nu_chb
    errors_u, errors_nu_seg, errors_nu_chb = initializer(nxvec)
    pool = mp.Pool(mp.cpu_count()-1, init_pool(errors_u, errors_nu_seg, errors_nu_chb))
...
def  calulate_all_errors_bkup(nxvec, find_anlyt_error_func, timeDuration):
    ...
    for indx, num_of_cells in enumerate(nxvec):
        ...
        setup_dict['center_spacing']  = max(delta_vect)
        three_time_phi = de_f_time(1, setup_dict, mesh, None, timeDuration)  #False  True
        errors_nu_chb[indx], _ = find_anlyt_error_func(mesh, three_time_phi[1][-1], timeDuration)
    return  errors_u, errors_nu_seg, errors_nu_chb




def plot_conv_acc(timeDuration, nxvec, errors_u, errors_nu_seg, errors_nu_chb):
    import numpy as np
    import matplotlib.pyplot as plt
